...
def _route_signal_chats(sig: dict) -> list[str]:
    ...
    vip_min_conf = _env_float("VIP_MIN_CONFIDENCE", 92)
    vip_min_rr = _env_float("VIP_MIN_RR", 3.0)
    public_min_conf = _env_float("PUBLIC_MIN_CONFIDENCE", 88)

    chats: list[str] = []

    if confidence >= vip_min_conf and rr >= vip_min_rr and vip_chat:
        chats.append(vip_chat)

    if confidence >= public_min_conf and public_chat:
    ...
    # remove duplicate chats, keep order
    out = []
    for c in chats:
        if c not in out:
            out.append(c)
