...
async def record_trade_accounting(
    ...
    *,
    ...
    if slippage is None:
        slippage = (
            pnlmath.slippage_cost(expected_exit, exit_price, quantity) if expected_exit else 0.0
        )
    ...
    row = LiveTradeAccounting(
        ...
        side=side,
        mode=mode,
        gross_pnl=bd.gross_pnl,
        commission=bd.commission,
        funding_fee=bd.funding_fee,
        slippage=bd.slippage,
        net_pnl=bd.net_pnl,
        net_roe=bd.net_roe,
        realized_pnl=gross_pnl,
        total_fees=bd.total_fees,
        estimate_quality=bd.estimate_quality,
        opened_at=opened_at,
        closed_at=closed_at,
    ...
    db.add(
        ExchangeFeeEvent(
            ...
            trade_id=trade_id,
            fee_type="COMMISSION",
            amount=bd.commission,
            mode=mode,
            estimated=not commission_known,
        )
    )
    if bd.slippage:
        db.add(
    ...
    await db.flush()
    logger.info(
        f"[accounting] user={user_id} {exchange} {symbol} net={bd.net_pnl} "
...
async def _roll_daily(
    db: AsyncSession, *, user_id: int, mode: str, breakdown: pnlmath.PnlBreakdown, when: datetime
) -> None:
    day = when.date()
    row = (
        await db.execute(
            select(DailyUserPnl).where(
                DailyUserPnl.user_id == user_id, DailyUserPnl.day == day, DailyUserPnl.mode == mode
            )
        )
    ).scalar_one_or_none()
    if row is None:
...
async def summary(db: AsyncSession, *, user_id: Optional[int] = None) -> dict:
    q = select(
        ...
        func.count(),
        func.coalesce(func.sum(LiveTradeAccounting.gross_pnl), 0.0),
...
def _trade_dict(t: LiveTradeAccounting) -> dict:
    return {
        ...
        "estimate_quality": t.estimate_quality,
...
async def list_trades(
    db: AsyncSession, *, user_id: Optional[int] = None, limit: int = 200
) -> list[dict]:
        ...
        out.append(
            {
                ...
                "funding_fee": round(d.funding_fee, 4),
...
async def user_summary(db: AsyncSession, user_id: int) -> dict:
    return {
        "user_id": user_id,
        **(await summary(db, user_id=user_id)),
