...
async def record_failure(
    ...
    *,
    ...
    decision = policy.decide_retry(
        error_class, 0, max_retries=5, recommended_delay=recommended_delay
    )
    row = OrderFailure(
        ...
        next_retry_at=(
            (_now() + timedelta(seconds=decision.delay_sec)) if decision.should_retry else None
        ),
    )
    db.add(row)
    await db.flush()
    logger.info(
        f"[order-fail] user={user_id} {exchange} {symbol} class={error_class} "
...
async def note_retry(db: AsyncSession, failure_id: int) -> dict:
    ...
    return {
        ...
        "needs_reconcile": decision.needs_reconcile,
...
async def recent_failure_count(db: AsyncSession, user_id: int, window_sec: int) -> int:
    ...
    return int(
        (
            await db.execute(
                select(func.count())
                .select_from(OrderFailure)
                .where(OrderFailure.user_id == user_id, OrderFailure.created_at >= since)
            )
        ).scalar_one()
        or 0
    )
...
async def circuit_breaker_tripped(db: AsyncSession, user_id: int) -> bool:
    ...
    if tripped:
        logger.warning(
            f"[order-fail] circuit breaker TRIPPED user={user_id} "
...
async def has_active_duplicate(db: AsyncSession, idempotency_key: str) -> bool:
    ...
    res = await db.execute(
        select(OrderFailure.id)
        .where(
            OrderFailure.idempotency_key == idempotency_key,
            OrderFailure.final_state.notin_([policy.RESOLVED, policy.FAILED]),
        )
        .limit(1)
    )
...
async def summary(db: AsyncSession) -> dict:
    ...
    for state, n in (
        await db.execute(
            select(OrderFailure.final_state, func.count()).group_by(OrderFailure.final_state)
        )
    ).all():
        by_state[state] = n
    ...
    for cls, n in (
        await db.execute(
            select(OrderFailure.error_class, func.count()).group_by(OrderFailure.error_class)
        )
    ).all():
        by_class[cls] = n
