...
ALL_ERROR_CLASSES = (
    INSUFFICIENT_BALANCE, PRECISION_ERROR, MIN_NOTIONAL, RATE_LIMIT, NETWORK_TIMEOUT,
    EXCHANGE_DOWN, ORDER_REJECTED, REDUCE_ONLY_REJECTED, TP_SL_FAILED, UNKNOWN,
)
...
def classify_error(message: str, *, code: str | int | None = None, is_tp_sl: bool = False) -> str:
    ...
    if ("unavailable" in msg or "service" in msg and "down" in msg or "503" in msg
            or "502" in msg or "system busy" in msg or "maintenance" in msg):
...
def decide_retry(
    ...
    *,
    ...
    if error_class == ORDER_REJECTED:
        return RetryDecision(False, 0.0, True, False, FAILED,
                             "Order rejected — manual review, no blind retry.")
    ...
    if error_class == RATE_LIMIT:
        ...
        return RetryDecision(True, float(delay), False, False, RETRY_SCHEDULED,
    ...
    if error_class == NETWORK_TIMEOUT:
        ...
        return RetryDecision(True, backoff_delay(retry_count), False, True, NEEDS_RECONCILE,
                             "Timeout — check order status, then retry with backoff.")
    if error_class == EXCHANGE_DOWN:
        return RetryDecision(True, backoff_delay(retry_count, base=5.0, cap=120.0), False, False,
                             RETRY_SCHEDULED, "Exchange down — retry with longer backoff.")
    if error_class == TP_SL_FAILED:
        ...
        return RetryDecision(True, delay, False, False, RETRY_SCHEDULED,
                             "TP/SL failed — retry immediately then with backoff.")

    return RetryDecision(False, 0.0, False, True, NEEDS_RECONCILE,
                         "Unclassified — reconcile before retry.")


def idempotency_key(*, user_id: int, exchange: str, symbol: str, side: str,
                    signal_id: int | str | None = None) -> str:
    """
...
def breaker_tripped(failure_times: list[float], now: float, *,
                    window_sec: int, threshold: int) -> bool:
    """
