"""Server-rendered HTML for the dashboard (extracted from server.py).

Pure presentation: page-shell + per-page builders + the static HTML
templates. Depends only on settings and the local escape/shell helpers
(no DB, no routers) so it imports cleanly without cycles. server.py
re-exports these names for the route modules.
"""
...
<link rel="icon" type="image/png" href="/static/brand/argus-quant-favicon.png"/>
...
/* NAV */
.nav{position:sticky;top:0;z-index:50;background:rgba(3,11,23,.74);backdrop-filter:blur(18px);border-bottom:1px solid rgba(255,255,255,.06)}.nav-in{height:64px;display:flex;align-items:center;justify-content:space-between;gap:28px}.brand{display:flex;align-items:center;gap:10px;max-width:190px;overflow:hidden}.logo-svg{width:67px;height:67px;filter:drop-shadow(0 0 26px rgba(24,242,139,.55)) drop-shadow(0 0 8px rgba(8,232,210,.3))}.brand-mark{width:44px;height:44px;flex:0 0 auto;display:block;filter:drop-shadow(0 0 10px rgba(52,211,153,.35))}.brand-word{font-weight:900;font-size:20px;letter-spacing:.5px;line-height:1.05;color:#fff;white-space:nowrap}.brand-word b{display:block;color:#34d399;font-size:11px;font-weight:800;letter-spacing:.25em}.nav-links{display:flex;align-items:center;gap:28px;margin-left:auto}.nav-links a{font-size:14px;font-weight:650;color:#d8e7f5}.nav-links a:hover{color:var(--green)}.nav-tg{display:inline-flex!important;align-items:center;gap:9px;background:linear-gradient(135deg,#10ddb6,#18f28b);color:#031117!important;border-radius:9px;padding:12px 22px;font-weight:900;box-shadow:0 10px 28px rgba(24,242,139,.24)}.nav-tg:before{content:'➤';font-size:14px}.nav-dc,.nav-admin,.live-pill{display:none!important}
/* HERO */
...
async function openAnalysis(id){const modal=document.getElementById('am-modal');modal.classList.add('open');document.getElementById('am-content').innerHTML='<p style="text-align:center;color:var(--muted);padding:40px">Loading…</p>';try{const [r,r2]=await Promise.all([fetch('/api/public/signal/'+id),fetch('/api/public/diagnostics/'+id)]);if(!r.ok)throw new Error('Signal not found');const d=await r.json();if(d.error)throw new Error(d.error);const diag=(r2&&r2.ok)?((await r2.json()).diagnostics||{}):{};const num=function(v){return (v===null||v===undefined||v===''||isNaN(v))?null:(+v);};const sc=d.side==='LONG'?'var(--green)':'var(--red)';const cc=d.confidence>=85?'var(--green)':d.confidence>=75?'var(--yellow)':'var(--red)';const reasons=d.reasons||[];const t1d=reasons.filter(x=>/^1[Dd]/i.test(x));const s4h=reasons.filter(x=>/^4[Hh]/i.test(x));const h1=reasons.filter(x=>/^1[Hh]/i.test(x));const m15=reasons.filter(x=>/^15[Mm]/i.test(x));const es=num(diag.entry_score);const entryList=m15.slice();if(es!==null)entryList.unshift('Entry timing score: '+es+' / 5');const fundingList=[];if(diag.funding_class)fundingList.push('Class: '+diag.funding_class);const fs=num(diag.funding_score);if(fs!==null)fundingList.push('Funding score: '+(fs>=0?'+':'')+fs);reasons.filter(x=>/^Funding[: ]/i.test(x)).forEach(function(x){fundingList.push(x);});const riskList=[];if(diag.tier)riskList.push('Tier: '+diag.tier);const mc=num(diag.effective_min_confidence);if(mc!==null)riskList.push('Min confidence: '+mc+'%');const mr=num(diag.effective_min_rr);if(mr!==null)riskList.push('Min RR: 1:'+mr);const sld=num(diag.sl_distance_percent);if(sld!==null)riskList.push('Stop distance: '+sld.toFixed(2)+'%');if(diag.stoploss_method)riskList.push('Stop method: '+diag.stoploss_method);const noData='<p style="color:var(--muted);font-size:12px;font-style:italic">Detailed diagnostics not available for this signal yet.</p>';function rList(arr){return arr.length?arr.map(s=>'<div class="am-reason"><div class="am-dot"></div><span>'+s+'</span></div>').join(''):noData;}document.getElementById('am-content').innerHTML='<div class="am-meta"><div class="am-mcard"><div class="am-mlbl">Symbol</div><div class="am-mval">'+d.symbol+'</div></div><div class="am-mcard"><div class="am-mlbl">Side</div><div class="am-mval" style="color:'+sc+'">'+d.side+'</div></div><div class="am-mcard"><div class="am-mlbl">Confidence</div><div class="am-mval" style="color:'+cc+'">'+d.confidence+'%</div></div><div class="am-mcard"><div class="am-mlbl">RR</div><div class="am-mval" style="color:var(--yellow)">1:'+d.risk_reward+'</div></div></div><div class="am-layer"><div class="am-layer-hdr"><span>1D</span> Trend Engine</div><div class="am-layer-body">'+rList(t1d)+'</div></div><div class="am-layer"><div class="am-layer-hdr"><span>4H</span> Market Structure</div><div class="am-layer-body">'+rList(s4h)+'</div></div><div class="am-layer"><div class="am-layer-hdr"><span>1H</span> Setup Engine</div><div class="am-layer-body">'+rList(h1)+'</div></div><div class="am-layer"><div class="am-layer-hdr"><span>15M</span> Entry Timing</div><div class="am-layer-body">'+rList(entryList)+'</div></div><div class="am-layer"><div class="am-layer-hdr"><span>💰</span> Funding Filter</div><div class="am-layer-body">'+rList(fundingList)+'</div></div><div class="am-layer"><div class="am-layer-hdr"><span>🛡</span> Risk Filter</div><div class="am-layer-body">'+rList(riskList)+'</div></div>';}catch(e){document.getElementById('am-content').innerHTML='<p style="color:var(--red);padding:20px">'+e.message+'</p>';}}
...
<body>
<div class="top">
  <div><h1 style="display:flex;align-items:center;gap:10px"><img src="/static/brand/argus-quant-mark.png" alt="Argus Quant" style="width:38px;height:38px;object-fit:contain;border-radius:8px"/>SaaS Platform Oversight</h1><div class="sub">Multi-user admin · read-only aggregation · credentials never exposed</div></div>
  <div>
...
<div class="wrap">
<aside class="side">
  <div class="logo"><img src="/static/brand/argus-quant-mark.png" alt="Argus Quant" style="width:48px;height:48px;object-fit:contain;border-radius:10px"/><div class="brand">ARGUS <span>QUANT</span></div></div>
  <div class="nav">
