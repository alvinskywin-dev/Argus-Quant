...
function modal(title, bodyHtml){
  closeModal();
  const o=h(`<div class="overlay show" id="modal"><div class="modal"><div class="mh"><h3>${esc(title)}</h3><button class="x">&times;</button></div><div class="mb">${bodyHtml}</div></div></div>`);
...
function renderShell(){
      ...
      <div class="topbar">
        ...
        <div class="right">
          ...
          <div class="userchip"><div class="avatar">${esc((ME.email||"?")[0].toUpperCase())}</div>
            <div><div style="font-weight:700">${esc(ME.username||ME.email.split("@")[0])}</div></div>
            ${badge(ME.role)}</div>
        </div>
      </div>
  ...
  // live gate badge (top-right)
  tryGet("/api/live/status").then(r=>{const b=$("#gatebadge");if(!b)return;if(r.data){b.outerHTML=`<span class="badge ${r.data.live_gate_open?"LIVE":"MOCK"}">${r.data.mode} MODE</span>`;}else b.outerHTML=`<span class="badge MOCK">MOCK MODE</span>`;});
}
...
function renderLanding(){
  ...
  const doLogin=async()=>{
    ...
    try{
      ...
      if(d.two_factor_required){$("#totpf").classList.remove("hide");$("#autherr").innerHTML=`<div class="alert info">Enter your 2FA code.</div>`;$("#loginbtn").disabled=false;return;}
...
function renderRegister(){
  ...
  box.innerHTML=`<h3>Create account</h3><div class="mut">Start with a paper-trading account.</div>
    ...
    <button class="btn primary" id="regbtn" style="width:100%;justify-content:center">Create Account</button>
  ...
  $("#regbtn").onclick=async()=>{
    ...
    try{
      ...
      TK.set(d.access_token,d.refresh_token); await boot();
    }catch(e){$("#autherr").innerHTML=`<div class="alert danger">${esc(e.status===404?"Auth disabled (AUTH_ENABLED=true).":e.detail)}</div>`;$("#regbtn").disabled=false;}
  };
...
PAGES.dashboard = async (v) => {
  v.innerHTML = `<div class="kpis" id="kpis">${Array(8).fill('<div class="sk kpi"></div>').join("")}</div>
    <div class="grid g2" style="margin-top:14px">
      ...
      <div class="card"><div class="card-h"><h3>System Health</h3></div><div class="card-b" id="health">${skel(3)}</div></div>
    </div>
    <div class="card" style="margin-top:14px"><div class="card-h"><h3>Recent Signals</h3><a href="#/analytics" class="btn sm">Analytics →</a></div><div id="recent">${skel(5)}</div></div>`;

  // KPIs (mix of public + admin/user)
  const [status, wr, regime] = await Promise.all([tryGet("/status"), tryGet("/api/public/winrate-analysis"), tryGet("/api/public/market-regime")]);
  const ov = ME.role==="ADMIN" ? await tryGet("/api/admin/overview") : {};
  const acct = await tryGet("/api/paper/account/");
  const auto = await tryGet("/api/auto/status");
  const O = ov.data, W = wr.data, S = status.data, A = acct.data, AU = auto.data;
  const winrate = W ? ((W.long_winrate+W.short_winrate)/2).toFixed(1)+"%" : "—";
  const kpis = [
    stat("Signals (sample)", W?W.sample_size:"—","best TF "+(W?W.best_timeframe:"—"),"📡"),
    stat("Avg Winrate", winrate, W?("long "+W.long_winrate+"% · short "+W.short_winrate+"%"):"","🎯"),
    stat("Your Open Positions", A?A.open_positions:"—", A?("equity "+money(A.equity)):"","📂"),
    stat("System Health", S?(S.status==="ok"?'<span class="pos">HEALTHY</span>':'<span class="neg">DEGRADED</span>'):"—", S?("universe "+S.universe):"","💚"),
    stat("Active Users", O?O.users.total:(ME.role==="ADMIN"?"—":"—"), O?("admins "+(O.users.by_role.ADMIN||0)):"admin only","👥"),
    stat("Connected Exchanges", O?O.exchange_accounts.connected:"—", O?Object.keys(O.exchange_accounts.by_exchange||{}).join(", "):"admin only","🔗"),
    stat("Auto-Trading Users", O?O.auto_trading_enabled_users:(AU?(AU.enabled?"You: ON":"You: OFF"):"—"), O?("kills "+O.user_kill_switches_active):"","🤖"),
    stat("Global Kill", O?(O.global_kill?'<span class="neg">ON</span>':'<span class="pos">OFF</span>'):"—", O?(O.live_gate_open?"gate OPEN":"gate closed"):"admin only","🛑"),
  ];
  ...
  // recent signals (admin audit as proxy of activity if available, else winrate buckets table)
  const rec=$("#recent");
  if(W && W.confidence_buckets){
    rec.innerHTML=tableWrap(["Confidence","Trades","Winrate"],
      W.confidence_buckets.map(b=>`<tr><td>${esc(b.label)}</td><td class="num">${b.total??b.count??"—"}</td><td class="num ${cls((b.winrate||0)-50)}">${num(b.winrate,1)}%</td></tr>`).join("")||`<tr><td colspan=3>${empty("—","No data")}</td></tr>`);
  } else rec.innerHTML=empty("📭","No recent signal analytics");
};

PAGES.analytics = async (v) => {
  v.innerHTML=`<div class="grid g2">
    <div class="card"><div class="card-h"><h3>Confidence vs Winrate</h3></div><div class="card-b"><canvas id="c1" height="150"></canvas></div></div>
    <div class="card"><div class="card-h"><h3>Long vs Short Winrate</h3></div><div class="card-b"><canvas id="c2" height="150"></canvas></div></div>
    <div class="card"><div class="card-h"><h3>RR Buckets</h3></div><div class="card-b"><canvas id="c3" height="150"></canvas></div></div>
    <div class="card"><div class="card-h"><h3>Summary</h3></div><div class="card-b" id="sum">${skel(4)}</div></div></div>`;
  const wr=await tryGet("/api/public/winrate-analysis");
  if(!wr.data){$("#sum").innerHTML=empty("📉","Analytics unavailable");return;}
  const W=wr.data;
  $("#sum").innerHTML=`<div class="kv"><span>Sample size</span><b>${W.sample_size}</b></div>
    ...
    <div class="kv"><span>Best timeframe</span><b>${esc(W.best_timeframe)}</b></div>`;
  if(!window.Chart){return;}
  const gridc="#17314b", txtc="#8fa8c7";
  const opt=(extra={})=>Object.assign({responsive:true,plugins:{legend:{labels:{color:txtc}}},scales:{x:{ticks:{color:txtc},grid:{color:gridc}},y:{ticks:{color:txtc},grid:{color:gridc}}}},extra);
  const cb=W.confidence_buckets||[];
  new Chart($("#c1"),{type:"bar",data:{labels:cb.map(b=>b.label),datasets:[{label:"Winrate %",data:cb.map(b=>b.winrate||0),backgroundColor:"#20f0c0aa"}]},options:opt()});
  new Chart($("#c2"),{type:"doughnut",data:{labels:["Long","Short"],datasets:[{data:[W.long_winrate,W.short_winrate],backgroundColor:["#22c55e","#ef4444"]}]},options:{responsive:true,plugins:{legend:{labels:{color:txtc}}}}});
  const rb=W.rr_buckets||[];
  new Chart($("#c3"),{type:"bar",data:{labels:rb.map(b=>b.label),datasets:[{label:"Winrate %",data:rb.map(b=>b.winrate||0),backgroundColor:"#f59e0baa"}]},options:opt()});
};
...
PAGES.paper = async (v) => {
  ...
  v.innerHTML=`<div class="kpis">
    ...
    <div class="card" id="ptab">${skel(4)}</div>`;
  ...
  $("#reset").onclick=async()=>{if(!confirm("Reset paper account to initial balance?"))return;try{await api("/api/paper/account/reset",{method:"POST"});toast("Account reset","ok");route();}catch(e){toast(e.detail,"bad");}};
  const tabs=v.querySelectorAll(".tabs button");
  const loadTab=async(t)=>{
    tabs.forEach(b=>b.classList.toggle("active",b.dataset.t===t));
    const box=$("#ptab"); box.innerHTML=skel(4);
    if(t==="open"){
      const p=await tryGet("/api/paper/account/positions?status=open");const rows=p.data||[];
      box.innerHTML=rows.length?tableWrap(["Symbol","Side","Entry","Mark","Qty","Lev","uPnL","ROE",""],
        rows.map(r=>`<tr><td><b>${esc(r.symbol)}</b></td><td>${badge(r.side)}</td><td class="num">${num(r.entry_price,4)}</td>
          <td class="num">${r.mark_price?num(r.mark_price,4):"—"}</td><td class="num">${num(r.quantity,4)}</td><td>${r.leverage}x</td>
          <td class="num ${cls(r.unrealized_pnl)}">${r.unrealized_pnl!=null?money(r.unrealized_pnl):"—"}</td>
          <td class="num ${cls(r.roe_pct)}">${r.roe_pct!=null?pct(r.roe_pct):"—"}</td>
          <td><button class="btn sm" data-pos="${r.id}">Detail</button></td></tr>`).join("")):empty("📭","No open positions","Copy a signal or open a position to start.");
      ...
      box.innerHTML=rows.length?tableWrap(["Symbol","Side","Entry","Exit","PnL","When"],
        rows.map(r=>`<tr><td><b>${esc(r.symbol)}</b></td><td>${badge(r.side)}</td><td class="num">${num(r.entry_price,4)}</td>
          <td class="num">${num(r.exit_price,4)}</td><td class="num ${cls(r.pnl_usdt)}">${money(r.pnl_usdt)}</td><td class="num">${ago(r.closed_at)} ago</td></tr>`).join("")):empty("📜","No closed trades yet");
    }
  };
...
PAGES.exchange = async (v) => {
  ...
  v.innerHTML=`<div class="alert info">🔒 API secrets are encrypted (AES-256-GCM) and never displayed. Withdrawal-enabled keys are rejected.</div>
    <div class="grid g2">${EX.map(([id,name])=>{
      ...
      return `<div class="card pad"><div class="row" style="justify-content:space-between;align-items:center">
        <div style="display:flex;align-items:center;gap:12px"><img src="/static/exchanges/${id}.svg" onerror="this.style.display='none'" style="width:34px;height:34px"><div><b style="font-size:16px">${name}</b><div class="sub">${acc?badge(acc.status):'<span class="badge muted">NOT CONNECTED</span>'}</div></div></div>
  ...
  v.querySelectorAll("[data-test]").forEach(b=>b.onclick=async()=>{try{const r=await api("/api/exchange/test",{method:"POST",body:{exchange:b.dataset.test}});toast("Test: "+(r.detail||r.status||"ok"),"ok");route();}catch(e){toast(e.detail,"bad");}});
  v.querySelectorAll("[data-disc]").forEach(b=>b.onclick=async()=>{if(!confirm("Disconnect "+b.dataset.disc+"? Encrypted keys are wiped."))return;try{await api("/api/exchange/disconnect",{method:"POST",body:{exchange:b.dataset.disc}});toast("Disconnected","ok");route();}catch(e){toast(e.detail,"bad");}});
};
function connectModal(ex,name){
  ...
  modal("Connect "+name,`<div id="cerr"></div>
    ...
    <div class="alert warn">Use a <b>trade + futures</b> key with <b>withdrawals disabled</b>. Withdrawal-enabled keys are rejected.</div>
    <button class="btn primary" id="cbtn" style="width:100%;justify-content:center">Connect & Validate</button>`);
  $("#cbtn").onclick=async()=>{
    ...
    $("#cbtn").disabled=true;
    try{await api("/api/exchange/connect",{method:"POST",body});closeModal();toast(name+" connected","ok");route();}
    catch(e){$("#cerr").innerHTML=`<div class="alert danger">${esc(e.detail)}</div>`;$("#cbtn").disabled=false;}
  };
}
...
PAGES.auto = async (v) => {
  ...
  v.innerHTML=`<div class="alert info">🤖 Auto-trading executes on your <b>paper account only</b> (DEMO). No real orders.</div>
    <div class="grid g2">
      <div class="card"><div class="card-h"><h3>Configuration</h3><label class="switch"><input type="checkbox" id="en" ${C.enabled?"checked":""}><span class="sl"></span></label></div>
        <div class="card-b">
          ...
          <div class="row"><div class="field" style="flex:1"><label>Max positions</label><input id="maxp" type="number" value="${C.max_positions}"></div>
            <div class="field" style="flex:1"><label>Max leverage</label><input id="maxl" type="number" value="${C.max_leverage}"></div></div>
          ...
          <div class="row"><div class="field" style="flex:1"><label>Break-even</label><select id="be"><option value="true" ${C.use_break_even?"selected":""}>On</option><option value="false" ${!C.use_break_even?"selected":""}>Off</option></select></div>
            <div class="field" style="flex:1"><label>BE trigger</label><select id="bet"><option ${C.break_even_trigger==="TP1"?"selected":""}>TP1</option><option ${C.break_even_trigger==="TP2"?"selected":""}>TP2</option></select></div></div>
    ...
    <div class="card" style="margin-top:14px"><div class="card-h"><h3>Execution History</h3></div><div id="exec">${skel(4)}</div></div>`;
  const save=async(patch)=>{try{await api("/api/auto/config",{method:"PUT",body:patch});toast("Saved","ok");}catch(e){toast(e.detail,"bad");}};
  $("#en").onchange=e=>save({enabled:e.target.checked});
  $("#save").onclick=()=>save({risk_per_trade_pct:+$("#risk").value,max_positions:+$("#maxp").value,max_leverage:+$("#maxl").value,min_confidence:+$("#minc").value,allowed_coins:$("#coins").value.trim(),use_break_even:$("#be").value==="true",break_even_trigger:$("#bet").value});
  ...
  $("#exec").innerHTML=rows.length?tableWrap(["When","Symbol","Action","Reason","Detail"],
    rows.slice(0,50).map(r=>`<tr><td class="num">${ago(r.created_at)} ago</td><td><b>${esc(r.symbol)}</b></td><td>${badge(r.action,"muted")}</td><td>${esc(r.reason)}</td><td class="sub">${esc(r.detail||"")}</td></tr>`).join("")):empty("📭","No executions yet");
};
...
PAGES.safety = async (v) => {
  ...
  v.innerHTML=`${blocked?`<div class="alert danger">⛔ Trading is currently blocked: ${S.global_kill?"GLOBAL emergency stop":S.kill_switch?"your kill switch is ON":S.disabled_reason||"locked out"}.</div>`:`<div class="alert ok">✅ Trading enabled — all safety checks passing.</div>`}
    ...
    <div class="grid g2" style="margin-top:14px">
      <div class="card"><div class="card-h"><h3>Kill Switches</h3></div><div class="card-b">
        ...
        <div class="sub" style="margin-top:8px">Your kill switch immediately stops your auto-trading.</div>
      </div></div>
      <div class="card"><div class="card-h"><h3>Limits</h3></div><div class="card-b">
        ...
        <div class="row"><div class="field" style="flex:1"><label>Max open</label><input id="mo" type="number" value="${C.max_open_positions}"></div>
          <div class="field" style="flex:1"><label>Max correlated</label><input id="mc" type="number" value="${C.max_correlated_positions}"></div></div>
  ...
  $("#kill")&&($("#kill").onclick=async()=>{if(!confirm("Activate your kill switch? Auto-trading stops."))return;try{await api("/api/safety/kill",{method:"POST"});toast("Kill switch ON","warn");route();}catch(e){toast(e.detail,"bad");}});
  $("#resume")&&($("#resume").onclick=async()=>{try{await api("/api/safety/resume",{method:"POST"});toast("Trading resumed","ok");route();}catch(e){toast(e.detail,"bad");}});
  $("#savelim").onclick=async()=>{try{await api("/api/safety/config",{method:"PUT",body:{max_daily_loss_pct:+$("#dl").value,max_weekly_loss_pct:+$("#wl").value,max_open_positions:+$("#mo").value,max_correlated_positions:+$("#mc").value,loss_streak_limit:+$("#ls").value}});toast("Limits saved","ok");}catch(e){toast(e.detail,"bad");}};
};

PAGES.live = async (v) => {
  const g=await tryGet("/api/live/status");
  ...
  const banner=G.live_gate_open
    ? `<div class="alert danger">⚡ LIVE GATE OPEN — real orders can be placed. (LIVE_TRADING_ENABLED & MOCK off)</div>`
    : `<div class="alert ok">🧪 MOCK MODE — all execution is simulated. No real orders. Gate requires LIVE_TRADING_ENABLED=true AND MOCK_EXCHANGE_MODE=false.</div>`;
  v.innerHTML=`${banner}
    <div class="row" style="margin:0 0 14px"><span class="badge ${G.live_gate_open?"LIVE":"MOCK"}" style="font-size:14px;padding:8px 18px">${G.mode} MODE</span>
    ...
    <div class="card" id="ltab">${skel(4)}</div>`;
  const tabs=v.querySelectorAll(".tabs button");
  const load=async(t)=>{tabs.forEach(b=>b.classList.toggle("active",b.dataset.t===t));const box=$("#ltab");box.innerHTML=skel(4);
    if(t==="pos"){const p=await tryGet("/api/live/positions");const r=p.data||[];
      box.innerHTML=r.length?tableWrap(["Mode","Exchange","Symbol","Side","Qty","Entry","Lev","Status","PnL"],
        r.map(x=>`<tr><td>${badge(x.mode)}</td><td>${esc(x.exchange)}</td><td><b>${esc(x.symbol)}</b></td><td>${badge(x.side)}</td><td class="num">${num(x.quantity,4)}</td><td class="num">${num(x.entry_price,4)}</td><td>${x.leverage}x</td><td>${badge(x.status)}</td><td class="num ${cls(x.realized_pnl)}">${money(x.realized_pnl)}</td></tr>`).join("")):empty("📭","No live positions");
    } else if(t==="ord"){const p=await tryGet("/api/live/orders");const r=p.data||[];
      box.innerHTML=r.length?tableWrap(["Mode","Exchange","Symbol","Side","Type","Qty","Status","When"],
        r.map(x=>`<tr><td>${badge(x.mode)}</td><td>${esc(x.exchange)}</td><td><b>${esc(x.symbol)}</b></td><td>${badge(x.side)}</td><td>${esc(x.order_type)}</td><td class="num">${num(x.quantity,4)}</td><td>${badge(x.status)}</td><td class="num">${ago(x.created_at)} ago</td></tr>`).join("")):empty("📭","No orders");
    } else {const p=await tryGet("/api/live/trades");const r=p.data||[];
      box.innerHTML=r.length?tableWrap(["Mode","Symbol","Side","Entry","Exit","PnL","When"],
        r.map(x=>`<tr><td>${badge(x.mode)}</td><td><b>${esc(x.symbol)}</b></td><td>${badge(x.side)}</td><td class="num">${num(x.entry_price,4)}</td><td class="num">${num(x.exit_price,4)}</td><td class="num ${cls(x.pnl_usdt)}">${money(x.pnl_usdt)}</td><td class="num">${ago(x.closed_at)} ago</td></tr>`).join("")):empty("📭","No trades");
    }};
...
PAGES.profile = async (v) => {
  ...
  $("#sess").innerHTML=rows.length?tableWrap(["Device","IP","Last seen",""],
    rows.map(r=>`<tr><td class="sub">${esc((r.device||"unknown").slice(0,40))}</td><td>${esc(r.ip||"—")}</td><td class="num">${ago(r.last_seen)} ago</td><td>${r.current?badge("ACTIVE"):""}</td></tr>`).join("")):empty("—","No active sessions");
};
...
PAGES.admin = async (v) => {
  ...
  v.innerHTML=`<div class="kpis">
    ...
    <div class="tabs"><button class="active" data-t="users">Users</button><button data-t="audit">Audit Log</button></div>
    <div class="card" id="atab">${skel(5)}</div>`;
  const tabs=v.querySelectorAll(".tabs button");
  const load=async(t)=>{tabs.forEach(b=>b.classList.toggle("active",b.dataset.t===t));const box=$("#atab");box.innerHTML=skel(5);
    if(t==="users"){const u=await tryGet("/api/admin/users?limit=200");const rows=(u.data&&u.data.users)||[];
      box.innerHTML=rows.length?tableWrap(["ID","Email","Role","Status","Exch","Auto","Kill","Last login",""],
        rows.map(r=>`<tr><td>${r.id}</td><td>${esc(r.email)}</td><td>${badge(r.role)}</td><td>${badge(r.status)}</td><td class="num">${r.connected_exchanges}</td>
          ...
          <td><button class="btn sm" data-u="${r.id}">View</button> ${r.status==="SUSPENDED"?`<button class="btn sm" data-act="${r.id}">Activate</button>`:`<button class="btn sm danger" data-sus="${r.id}">Suspend</button>`}</td></tr>`).join("")):empty("—","No users");
      ...
      box.innerHTML=rows.length?tableWrap(["When","User","Exchange","Symbol","Action","Result","Mode"],
        rows.map(r=>`<tr><td class="num">${ago(r.created_at)} ago</td><td>${r.user_id??"—"}</td><td>${esc(r.exchange)}</td><td>${esc(r.symbol||"—")}</td><td>${esc(r.action)}</td><td>${badge(r.result)}</td><td>${badge(r.mode)}</td></tr>`).join("")):empty("📭","No audit rows");
    }};
...
async function adminUser(id){try{const d=await api("/api/admin/users/"+id);const p=d.profile;
  modal(p.email, `<div class="kv"><span>Role / Status</span><span>${badge(p.role)} ${badge(p.status)}</span></div>
    ...
    <h4 style="margin:16px 0 6px">Open Positions (${d.open_positions.length})</h4>${d.open_positions.map(o=>`<div class="kv"><span>${esc(o.symbol)} ${badge(o.side)} ${badge(o.mode)}</span><span class="sub">${num(o.quantity,4)} @ ${num(o.entry_price,4)}</span></div>`).join("")||'<div class="sub">None</div>'}`);
}catch(e){toast(e.detail,"bad");}}
async function adminStatus(id,status){if(!confirm(status+" user "+id+"?"))return;try{await api("/api/admin/users/"+id+"/status",{method:"PUT",body:{status}});toast("User "+status,"ok");route();}catch(e){toast(e.detail,"bad");}}
...
async function route(){
  ...
  setActive(r);
  const v=$("#view"); v.innerHTML=skel(6);
  try{ await PAGES[r](v); }
  catch(e){ if(e.status===401){logout();return;} v.innerHTML=`<div class="card pad">${empty("⚠️","Failed to load page",e.detail||"")}</div>`; }
}
