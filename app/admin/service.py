...
async def overview(db: AsyncSession) -> dict:
    ...
    exch_by_exchange = await _group_counts(
        db, ExchangeAccount.exchange,
    )
    connected_exch = await _count(
        db, select(func.count()).select_from(ExchangeAccount)
        .where(ExchangeAccount.status == "CONNECTED"))

    open_positions = await _count(
        db, select(func.count()).select_from(LivePosition)
    ...
    auto_enabled = await _count(
        db, select(func.count()).select_from(AutoTradeConfig)
        .where(AutoTradeConfig.enabled.is_(True)))
    kill_switches = await _count(
        db, select(func.count()).select_from(SafetyState)
        .where(SafetyState.kill_switch.is_(True)))

    realized = (await db.execute(
        select(func.coalesce(func.sum(LiveTrade.pnl_usdt), 0.0)))).scalar() or 0.0
...
async def list_users(db: AsyncSession, *, limit: int = 100, offset: int = 0,
                     status: Optional[str] = None, role: Optional[str] = None) -> dict:
    q = select(AuthUser)
    if status:
    ...
    exch_counts = await _per_user_count(db, ExchangeAccount, ids,
                                        where=ExchangeAccount.status == "CONNECTED")
...
async def _per_user_flag(db, model, ids, column) -> dict[int, bool]:
    ...
    rows = (await db.execute(
        select(model.user_id, column).where(model.user_id.in_(ids)))).all()
...
async def user_detail(db: AsyncSession, user_id: int) -> dict:
    ...
    accounts = list((await db.execute(
        ...
        .order_by(ExchangeAccount.exchange))).scalars().all())
    auto = (await db.execute(
        select(AutoTradeConfig).where(AutoTradeConfig.user_id == user_id))).scalar_one_or_none()
    state = (await db.execute(
        select(SafetyState).where(SafetyState.user_id == user_id))).scalar_one_or_none()
    positions = list((await db.execute(
        select(LivePosition).where(LivePosition.user_id == user_id, LivePosition.status == "OPEN")
        .order_by(LivePosition.opened_at.desc()))).scalars().all())

    return {
        "profile": {
            "id": user.id, "email": user.email, "username": user.username,
            "role": user.role, "status": user.status, "is_verified": user.is_verified,
        ...
        "exchange_accounts": [{
            "exchange": a.exchange, "label": a.label, "status": a.status,
            "api_key_last4": a.api_key_last4, "can_trade": a.can_trade,
            "can_futures": a.can_futures, "can_withdraw": a.can_withdraw,
        } for a in accounts],
        "auto_trade": None if auto is None else {
            "enabled": auto.enabled, "max_positions": auto.max_positions,
            "max_leverage": auto.max_leverage, "risk_per_trade_pct": auto.risk_per_trade_pct,
            "allowed_exchanges": auto.allowed_exchanges, "min_confidence": auto.min_confidence,
        },
        "safety_state": None if state is None else {
        ...
        "open_positions": [{
            ...
            "quantity": p.quantity, "entry_price": p.entry_price, "leverage": p.leverage,
            "mode": p.mode, "opened_at": p.opened_at.isoformat() if p.opened_at else None,
        } for p in positions],
    }
...
async def audit_feed(db: AsyncSession, *, limit: int = 100, user_id: Optional[int] = None) -> list[dict]:
    ...
    return [{
        "id": r.id, "user_id": r.user_id, "exchange": r.exchange, "symbol": r.symbol,
        "action": r.action, "result": r.result, "mode": r.mode,
