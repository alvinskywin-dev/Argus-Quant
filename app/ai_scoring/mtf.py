...
def evaluate_pipeline(
    ...
    d1s = d1.structure
    d1_struct_ok = (side == "LONG" and (d1s.bos_bull or d1s.mss_bull)) or (
    ...
    trend_reasons: List[str] = [f"1D trend {side}: EMA50 {'>' if side == 'LONG' else '<'} EMA200"]
    if d1_struct_ok:
    ...
    setup_map = {
        ...
        "EMA_align": (
            (side == "LONG" and h1.ema_fast > h1.ema_slow and h1.last_close > h1.ema_slow)
            or (side == "SHORT" and h1.ema_fast < h1.ema_slow and h1.last_close < h1.ema_slow)
        ),
    ...
    entry_factors: Dict[str, bool] = {
        ...
        "EMA pullback": (
            (side == "LONG" and m15.ema_fast > m15.ema_slow and m15.last_close > m15.ema_slow)
            or (side == "SHORT" and m15.ema_fast < m15.ema_slow and m15.last_close < m15.ema_slow)
        ),
        "VWAP reclaim": (side == "LONG" and m15.above_vwap)
        or (side == "SHORT" and not m15.above_vwap),
    }
    ...
    all_reasons = list(
        dict.fromkeys(trend_reasons + struct_reasons + setup_reasons + entry_reasons)
    )
