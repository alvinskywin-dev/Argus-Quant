...
class BitgetAdapter(ExchangeAdapter):
    ...
    async def _request(self, method: str, path: str, params: Optional[dict] = None,
        ...
        async with session.request(method, f"{_BASE}{request_path}", data=body_str or None, headers=headers) as resp:
    ...
    async def get_balance(self, asset: str = "USDT") -> BalanceInfo:
        ...
        for a in data or []:
            if a.get("marginCoin") == asset:
                return BalanceInfo(asset=asset, balance=float(a.get("accountEquity", 0) or 0),
                                   available=float(a.get("available", 0) or 0), mode=MODE_LIVE)
        return BalanceInfo(asset=asset, balance=0.0, available=0.0, mode=MODE_LIVE)

    async def get_positions(self) -> list[PositionInfo]:
        data = await self._request("GET", "/api/v2/mix/position/all-position", {"productType": _PRODUCT})
        ...
        for p in data or []:
            ...
            out.append(PositionInfo(
                symbol=p.get("symbol", ""), side="LONG" if p.get("holdSide") == "long" else "SHORT",
                qty=total, entry_price=float(p.get("openPriceAvg", 0) or 0),
                leverage=int(float(p.get("leverage", 1) or 1)), margin_type=p.get("marginMode", "isolated"),
                unrealized_pnl=float(p.get("unrealizedPL", 0) or 0),
                liquidation_price=float(p.get("liquidationPrice", 0) or 0), mode=MODE_LIVE))
        return out

    async def set_leverage(self, symbol: str, leverage: int) -> None:
        await self._request("POST", "/api/v2/mix/account/set-leverage", body={
            "symbol": symbol.upper(), "productType": _PRODUCT, "marginCoin": "USDT", "leverage": str(leverage)})

    async def set_margin_type(self, symbol: str, margin_type: str) -> None:
        mode = "isolated" if margin_type.lower() == "isolated" else "crossed"
        try:
            await self._request("POST", "/api/v2/mix/account/set-margin-mode", body={
                "symbol": symbol.upper(), "productType": _PRODUCT, "marginCoin": "USDT", "marginMode": mode})
    ...
    async def open_order(self, *, symbol: str, side: str, qty: float, order_type: str = "MARKET",
        ...
        body = {
            "symbol": symbol.upper(), "productType": _PRODUCT, "marginCoin": "USDT",
            "size": str(qty), "side": "buy" if side.upper() == "BUY" else "sell",
    ...
    async def set_tp_sl(self, *, symbol: str, side: str, qty: float, take_profit: Optional[float] = None,
        ...
        for plan, trigger in (("pos_profit", take_profit), ("pos_loss", stop_loss)):
            if not trigger:
                continue
            await self._request("POST", "/api/v2/mix/order/place-tpsl-order", body={
                "symbol": symbol.upper(), "productType": _PRODUCT, "marginCoin": "USDT",
                "planType": plan, "triggerPrice": str(trigger), "holdSide": side.lower()})
            out.append(OrderResult(order_id="", symbol=symbol, side=close_side, type=plan.upper(),
                                   status="NEW", qty=qty, reduce_only=True, mode=MODE_LIVE))
        return out
    ...
    async def get_order_status(self, *, symbol: str, order_id: str) -> OrderResult:
        ...
        return OrderResult(order_id=order_id, symbol=symbol, side=row.get("side", ""),
                           type=row.get("orderType", "MARKET").upper(), status=row.get("state", "NEW"),
                           filled_qty=float(row.get("baseVolume", 0) or 0),
                           avg_price=float(row.get("priceAvg", 0) or 0), mode=MODE_LIVE, raw=row)
