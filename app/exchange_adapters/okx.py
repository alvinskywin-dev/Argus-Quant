...
class OKXAdapter(ExchangeAdapter):
    ...
    async def _request(self, method: str, path: str, params: Optional[dict] = None,
        ...
        async with session.request(method, f"{_BASE}{path}", data=body_str or None, headers=headers) as resp:
    ...
    async def get_balance(self, asset: str = "USDT") -> BalanceInfo:
        ...
        for d in details:
            if d.get("ccy") == asset:
                return BalanceInfo(asset=asset, balance=float(d.get("eq", 0)),
                                   available=float(d.get("availBal", 0)), mode=MODE_LIVE)
    ...
    async def get_positions(self) -> list[PositionInfo]:
        ...
        for p in data or []:
            ...
            out.append(PositionInfo(
                symbol=p.get("instId", ""), side="LONG" if p.get("posSide") == "long" or pos > 0 else "SHORT",
                qty=abs(pos), entry_price=float(p.get("avgPx", 0) or 0),
                leverage=int(float(p.get("lever", 1) or 1)), margin_type=p.get("mgnMode", "cross"),
                unrealized_pnl=float(p.get("upl", 0) or 0),
                liquidation_price=float(p.get("liqPx", 0) or 0), mode=MODE_LIVE))
        return out
    ...
    async def open_order(self, *, symbol: str, side: str, qty: float, order_type: str = "MARKET",
        ...
        body = {
            "instId": to_inst_id(symbol), "tdMode": "isolated",
            "side": side.lower(), "ordType": order_type.lower(), "sz": str(qty),
        }
        if order_type == "LIMIT":
            body["px"] = str(price)
        if reduce_only:
        ...
        return OrderResult(order_id=str(row.get("ordId", "")), symbol=symbol, side=side,
                           type=order_type, status="NEW", qty=qty, reduce_only=reduce_only,
                           mode=MODE_LIVE, raw=row)
    ...
    async def set_tp_sl(self, *, symbol: str, side: str, qty: float, take_profit: Optional[float] = None,
        ...
        algo = {"instId": to_inst_id(symbol), "tdMode": "isolated", "side": close_side.lower(),
                "ordType": "oco", "sz": str(qty), "reduceOnly": "true"}
        if take_profit:
        ...
        if take_profit or stop_loss:
            ...
            row = data[0] if isinstance(data, list) and data else {}
            out.append(OrderResult(order_id=str(row.get("algoId", "")), symbol=symbol, side=close_side,
                                   type="OCO", status="NEW", qty=qty, reduce_only=True, mode=MODE_LIVE))
        return out

    async def get_order_status(self, *, symbol: str, order_id: str) -> OrderResult:
        data = await self._request("GET", "/api/v5/trade/order",
                                   {"instId": to_inst_id(symbol), "ordId": order_id})
        row = data[0] if isinstance(data, list) and data else {}
        return OrderResult(order_id=order_id, symbol=symbol, side=row.get("side", ""),
                           type=row.get("ordType", "MARKET").upper(), status=row.get("state", "NEW"),
                           filled_qty=float(row.get("accFillSz", 0) or 0),
                           avg_price=float(row.get("avgPx", 0) or 0), mode=MODE_LIVE, raw=row)
