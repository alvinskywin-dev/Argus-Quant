...
ALL_ISSUE_TYPES = (
    DB_POSITION_MISSING_ON_EXCHANGE, EXCHANGE_POSITION_MISSING_IN_DB,
    TP_SL_MISSING_ON_EXCHANGE, TP_SL_MISSING_IN_DB, SIZE_MISMATCH, ENTRY_MISMATCH,
    LEVERAGE_MISMATCH, MODE_MISMATCH, SIDE_MISMATCH, STATUS_MISMATCH,
    UNKNOWN_ORDER, ORPHAN_POSITION,
)
    ...
    *,
    ...
    if ex_protection is not None:
        ...
        if db_protected and not ex_protected:
            issues.append(DriftIssue(
                TP_SL_MISSING_ON_EXCHANGE, SEV_CRITICAL, symbol,
                "DB expects TP/SL but the exchange has none — position is "
                "UNPROTECTED. Mark UNSAFE and retry TP/SL placement.",
                db_state=dbp, exchange_state=ex_protection))
        elif ex_protected and not db_protected:
            issues.append(DriftIssue(
                TP_SL_MISSING_IN_DB, SEV_INFO, symbol,
                "Exchange has TP/SL orders not tracked in the DB. Attach order "
                "references.",
                db_state=dbp, exchange_state=ex_protection))
...
def _pos_to_dict(p: LivePosition) -> dict:
    return {
        "side": p.side, "qty": p.quantity, "entry_price": p.entry_price,
        "leverage": p.leverage, "margin_type": p.margin_type, "status": p.status,
    }


def _ex_pos_to_dict(pi) -> dict:
    return {
        "side": pi.side, "qty": pi.qty, "entry_price": pi.entry_price,
        "leverage": pi.leverage, "margin_type": pi.margin_type,
    }


async def _existing_unresolved(db: AsyncSession, user_id: int, exchange: str,
                               symbol: str, issue_type: str) -> bool:
    res = await db.execute(
        select(ReconciliationIssue.id).where(
...
async def reconcile_exchange_account(
    db: AsyncSession, *, user_id: int, exchange: str, persist: bool = True,
) -> dict:
    ...
    rows = (await db.execute(
        select(LivePosition).where(
            ...
            LivePosition.status == "OPEN"))).scalars().all()
    ...
    for symbol in set(db_by_symbol) | set(ex_by_symbol):
        ...
        ex_protection = ex_prot.get(symbol, {"has_tp": False, "has_sl": False}) if ex_orders else None
        drift.extend(reconcile_symbol(
            symbol, db_pos, ex_pos, ex_protection=ex_protection))
    ...
    if persist:
        for di in drift:
            ...
            db.add(ReconciliationIssue(
                user_id=user_id, exchange=exchange, symbol=di.symbol, mode=mode,
                severity=di.severity, issue_type=di.issue_type,
                db_state=json.dumps(di.db_state), exchange_state=json.dumps(di.exchange_state),
    ...
    return {
        "user_id": user_id, "exchange": exchange, "mode": mode,
        "db_positions": len(db_by_symbol), "exchange_positions": len(ex_by_symbol),
        "issues_found": len(drift), "issues_created": created,
...
async def reconcile_user(db: AsyncSession, *, user_id: int, persist: bool = True) -> dict:
    ...
    results = [await reconcile_exchange_account(
        db, user_id=user_id, exchange=ex, persist=persist) for ex in exchanges]
    return {
        "user_id": user_id, "exchanges": exchanges,
