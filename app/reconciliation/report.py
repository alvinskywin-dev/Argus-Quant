...
def _issue_dict(i: ReconciliationIssue) -> dict:
    def _load(s):
        try:
            ...
            return {}
    return {
        "id": i.id, "user_id": i.user_id, "exchange": i.exchange, "symbol": i.symbol,
        "mode": i.mode, "severity": i.severity, "issue_type": i.issue_type,
        "db_state": _load(i.db_state), "exchange_state": _load(i.exchange_state),
        "recommended_action": i.recommended_action, "resolved": i.resolved,
...
async def summary(db: AsyncSession, *, user_id: Optional[int] = None) -> dict:
    ...
    q = select(ReconciliationIssue.severity, ReconciliationIssue.resolved,
               func.count()).group_by(ReconciliationIssue.severity, ReconciliationIssue.resolved)
    ...
    last = (await db.execute(
        select(func.max(ReconciliationIssue.created_at)))).scalar_one_or_none()
    return {
        "total_issues": total, "unresolved": unresolved, "by_severity": by_sev,
        "last_issue_at": last.isoformat() if last else None,
    }


async def list_issues(db: AsyncSession, *, user_id: Optional[int] = None,
                      resolved: Optional[bool] = None, limit: int = 200) -> list[dict]:
