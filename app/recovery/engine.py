...
async def _alert_admin(user_id: Optional[int], exchange: str, symbol: str,
    ...
    try:
        async with SessionLocal() as adb:
            adb.add(LiveAuditLog(
                user_id=user_id, exchange=exchange, symbol=symbol or "",
                action="RECOVERY", result=result, mode=_mode(), detail=(detail or "")[:512]))
            await adb.commit()
...
async def _record_issue(db: AsyncSession, *, user_id: int, exchange: str, symbol: str,
                        issue_type: str, severity: str, recommended: str,
                        db_state: dict, exchange_state: dict) -> None:
    if await recon._existing_unresolved(db, user_id, exchange, symbol, issue_type):
        return
    db.add(ReconciliationIssue(
        user_id=user_id, exchange=exchange, symbol=symbol, mode=_mode(),
        severity=severity, issue_type=issue_type,
        db_state=json.dumps(db_state), exchange_state=json.dumps(exchange_state),
        recommended_action=recommended))


# ── mark unsafe ─────────────────────────────────────────────────────

async def mark_position_unsafe(db: AsyncSession, position_id: int, reason: str) -> Optional[LivePosition]:
    ...
    await db.flush()
    await _alert_admin(pos.user_id, pos.exchange, pos.symbol,
                       f"POSITION MARKED UNSAFE: {reason}", result="FAIL")
    return pos
...
async def sync_tp_sl_for_position(db: AsyncSession, position_id: int, *,
                                  attempt_replace: bool = True) -> dict:
    """
    ...
    try:
        ...
        adapter = resolve_adapter(
            pos.exchange, api_key=creds["api_key"], api_secret=creds["api_secret"],
            passphrase=creds.get("passphrase"))
    except vault.VaultError:
        pos.tp_sl_status = tp_sl.UNKNOWN
        await db.flush()
        return {"position_id": position_id, "tp_sl_status": tp_sl.UNKNOWN, "reason": "no credentials"}
    ...
    try:
        ...
        if prot["has_tp"] is None:   # couldn't read orders
            pos.tp_sl_status = tp_sl.UNKNOWN
            await db.flush()
            return {"position_id": position_id, "tp_sl_status": tp_sl.UNKNOWN, "reason": "orders unreadable"}

        status = tp_sl.compute_tp_sl_status(
            bool(prot["has_tp"]), bool(prot["has_sl"]),
            ...
            for _ in range(max(1, settings.tp_sl_retry_max)):
                ...
                try:
                    await adapter.set_tp_sl(
                        ...
                        take_profit=pos.take_profit if (expected_tp and not prot["has_tp"]) else None,
                    ...
                    await _alert_admin(pos.user_id, pos.exchange, pos.symbol,
                                       f"TP/SL retry failed: {exc}", result="FAIL")
        ...
        pos.requires_review = True
        await _record_issue(
            db, user_id=pos.user_id, exchange=pos.exchange, symbol=pos.symbol,
            issue_type=recon.TP_SL_MISSING_ON_EXCHANGE, severity=recon.SEV_CRITICAL,
        ...
        await db.flush()
        await _alert_admin(pos.user_id, pos.exchange, pos.symbol,
                           f"UNPROTECTED position: {pos.unsafe_reason}", result="FAIL")
        return {"position_id": position_id, "tp_sl_status": pos.tp_sl_status,
                "retries": retries, "unsafe": True}
    finally:
        await adapter.close()
...
async def recover_user_positions(db: AsyncSession, *, user_id: int) -> dict:
    ...
    accounts = (await db.execute(
        select(ExchangeAccount).where(
            ...
            ExchangeAccount.status == "CONNECTED"))).scalars().all()
    ...
    for acc in accounts:
        ...
        db_rows = (await db.execute(
            select(LivePosition).where(
                ...
                LivePosition.status.in_(["OPEN", "RECOVERED"])))).scalars().all()
        ...
        try:
            ...
            logger.warning(f"[recovery] user={user_id} {exchange} positions read failed: {exc!s:.120}")
            continue
        ...
        for symbol, pi in ex_by_symbol.items():
            if symbol in db_by_symbol:
                continue
            db.add(LivePosition(
                user_id=user_id, exchange=exchange, symbol=symbol, side=pi.side,
                quantity=pi.qty, entry_price=pi.entry_price, leverage=pi.leverage,
                margin_type=pi.margin_type, status="RECOVERED", mode=_mode(),
                tp_sl_status=tp_sl.UNKNOWN, requires_review=True, recovered_at=_now(),
                last_reconciled_at=_now()))
            recovered += 1
            reviewed += 1
            await _record_issue(
                db, user_id=user_id, exchange=exchange, symbol=symbol,
                issue_type=recon.EXCHANGE_POSITION_MISSING_IN_DB, severity=recon.SEV_CRITICAL,
                recommended="Imported as RECOVERED; review before auto-managing.",
                db_state={}, exchange_state=recon._ex_pos_to_dict(pi))
            await _alert_admin(user_id, exchange, symbol,
                               "Orphan exchange position imported as RECOVERED")
        ...
        for symbol, p in db_by_symbol.items():
            ...
            await _record_issue(
                db, user_id=user_id, exchange=exchange, symbol=symbol,
                issue_type=recon.DB_POSITION_MISSING_ON_EXCHANGE, severity=recon.SEV_CRITICAL,
                recommended="Position not on exchange; marked CLOSED_UNKNOWN for review.",
                db_state=recon._pos_to_dict(p), exchange_state={})
            await _alert_admin(user_id, exchange, symbol,
                               "DB position missing on exchange -> CLOSED_UNKNOWN", result="FAIL")
    ...
    return {
        "user_id": user_id, "recovered": recovered, "closed_unknown": closed_unknown,
        "tp_sl_synced": synced, "unsafe": unsafe, "requires_review": reviewed,
    }


async def recover_all_positions(db: AsyncSession) -> dict:
    """Recover every user that has a CONNECTED exchange account."""
    user_ids = sorted({uid for (uid,) in (await db.execute(
        select(ExchangeAccount.user_id).where(
            ExchangeAccount.status == "CONNECTED"))).all()})
