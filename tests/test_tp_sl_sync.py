...
def test_sl_only_position_is_synced_when_sl_present():
    ...
    assert tp_sl.compute_tp_sl_status(has_tp=False, has_sl=True, expected_tp=False) == tp_sl.SYNCED


def test_tp_only_position_missing_tp():
    assert (
        tp_sl.compute_tp_sl_status(has_tp=False, has_sl=False, expected_sl=False)
        == tp_sl.MISSING_TP
    )
