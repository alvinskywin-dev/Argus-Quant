...
def test_net_pnl_subtracts_all_costs():
    bd = m.compute_net_pnl(
        100.0, commission=2.0, funding_fee=1.0, slippage=0.5,
        commission_known=True, funding_known=True, slippage_known=True)
...
def test_net_roe_uses_margin():
    bd = m.compute_net_pnl(50.0, commission=0.0, margin=500.0,
                           commission_known=True, funding_known=True, slippage_known=True)
...
def test_funding_convention_positive_is_paid():
    ...
    paid = m.compute_net_pnl(10.0, funding_fee=2.0, funding_known=True,
                             commission_known=True, slippage_known=True)
...
def test_funding_convention_negative_is_received():
    ...
    received = m.compute_net_pnl(10.0, funding_fee=-2.0, funding_known=True,
                                 commission_known=True, slippage_known=True)
    assert received.net_pnl == 12.0      # 10 - (-2) = 12


def test_estimate_quality_partial_when_some_unknown():
    bd = m.compute_net_pnl(100.0, commission=2.0, commission_known=False,
                           funding_known=True, slippage_known=True)
...
def test_loss_is_negative_net():
    bd = m.compute_net_pnl(-20.0, commission=1.0, commission_known=True,
                           funding_known=True, slippage_known=True)
