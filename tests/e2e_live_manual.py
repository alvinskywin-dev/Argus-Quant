...
async def _purge(email):
    async with get_session() as db:
        u = (await db.execute(select(AuthUser).where(AuthUser.email == email.lower()))).scalar_one_or_none()
        if u:
            await db.delete(u)
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as c:
        await c.post("/api/auth/register", json={"email": "live@example.com", "password": "Sup3rSecret!"})
        r = await c.post("/api/auth/login", json={"email": "live@example.com", "password": "Sup3rSecret!"})
        ...
        r = await c.post("/api/live/open", headers=h, json={
            "exchange": "binance", "symbol": "BTCUSDT", "side": "LONG",
            "notional_usdt": 5000, "entry_price": 50000, "leverage": 10,
            "margin_type": "isolated", "take_profit": 55000, "stop_loss": 48000})
        ...
        # close at +10% -> +500 pnl
        r = await c.post("/api/live/close", headers=h, json={"position_id": pos_id, "exit_price": 55000})
        ...
        r = await c.post("/api/live/open", headers=h, json={
            "exchange": "binance", "symbol": "ETHUSDT", "side": "LONG",
            "notional_usdt": 1000, "entry_price": 3000, "leverage": 5})
        ...
        async with get_session() as db:
            rows = (await db.execute(
                select(LiveAuditLog).where(LiveAuditLog.user_id == uid))).scalars().all()
            audit = sorted({(a.action, a.result) for a in rows})
            assert all(a.mode == "MOCK" for a in rows)
        print("audit:", audit)
        assert ("OPEN", "OK") in audit and ("CLOSE", "OK") in audit and ("OPEN", "REJECTED") in audit
